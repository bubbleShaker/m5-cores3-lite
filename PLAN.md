...
### 🔴 次にやること: フレームを1ファイルにパックする（#170）
**動画は全長で再生できているが、実効 1.1fps しか出ておらず実用にならない。**
原因は #169 で特定済み: **FAT32 のファイル名解決コスト**。1ディレクトリに 2,355 ファイルあり、
名前の解決が線形走査になるため、フレーム番号が進むほど 1 枚の描画が遅くなる。

| 方式 | 1フレーム | 実効fps | 判定 |
|---|---|---|---|
| フラット配置（現状） | 58ms（先頭）〜1,051ms（終盤） | 17〜1.0 | 終盤で破綻 |
| サブディレクトリ分割（100枚ずつ） | 約880ms（均一） | 1.1 | **試して失敗・破棄済み** |
| パック方式（#170） | 目標 100ms 未満 | 目標 10 | **これをやる** |
...
**設計は #170 に全部書いてある**（ファイル形式・索引の持ち方・純粋関数・互換性）。要点だけ:
- `frames.bin` の先頭に固定長8バイト（offset/length の uint32 LE）の索引、その後に JPEG を連結
- 端末は入場時に1回 open し、`File` を保持したまま seek→read→`drawJpg(buf,len)`
- 索引（約19KB）は PSRAM に丸ごと載せる
- 純粋関数 `video_pack_entry` を TDD（索引の読み違いが一番壊れやすい。範囲外も必ずテスト）
- `meta.txt` に `pack=` が無ければ従来方式で読む（旧アセット互換）
...
1. **アセットを変換**（PC のみ・実機不要）
   - `python tools/video2frames.py <URL または動画ファイル> --name sample`
...
3. **転送**
   ...
   - `robocopy video\sample D:\video\sample /E` （終了コード 1 は「コピー成功」。エラーではない）
...
### 次にやること
1. **全長素材で PSRAM 上限を確かめる**（上記「🔴 未確定」）→ 2c-2 の要否を確定させる
