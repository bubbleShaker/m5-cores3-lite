...
### 🔴 次にやること（#176・計装は入れた／実測が実機待ち）

**#176 30fps 化の可否を判断する（内訳を実測する）**
30fps の予算は 33.3ms だが、現在は最良値 46ms で**既に予算超過**。どこを削れば効くかは
`draw total`（SD読み / JPEGデコード / LCD転送）の内訳で決まる。

**✅ 計装は済んだ（実機なしでビルド確認済み・実測は実機接続後）。** 実機が今すぐ使えないため、
計装コードだけ先に入れてビルドを通した（手戻り最小化）。焼いてシリアルを見れば数字が出る状態。

計装の中身（`src/main.cpp`）:
- **再生ループの内訳** … `videoDrawFromPack` を `micros()` で seek+read（SD読み）と drawJpg
  （デコード+LCD転送）に割り、200 フレーム間引きで出す:
  `[video] draw idx=.. total=..us sd=..us draw=..us jpg=..B pack=..`
  （`jpg` は JPEG バイト数。小さい/大きいフレームでデコード時間の差を読むため）
- **LCD 転送のみ＋実クロック** … 再生開始時に1回 `videoBenchLcd`。フルスクリーン Sprite を
  `pushSprite` 30 回平均＝転送のみ、`getBus()->getClock()` でバス実クロック:
  `[video] bench: lcd clk=..Hz push=..us/frame (.. img=..)`（`img=1` なら実絵で測れた）
- **デコード時間 = draw − push** として分離できる。

**実機接続後にやること**: 焼いて再生し、上記2ログを読む。判断材料は下表（Issue #176）。
結論（全画面 320×240 のまま 30fps 可能か／落としどころ）をここに追記する。

⚠ **推測で解像度を下げに行かないこと。** #169 で実測値の解釈を誤ってサブディレクトリ分割という
逆効果な実装をした前科がある。数字を読んでから決める。

未検証の仮説（このログで確かめる）: **LCD と SD が同一 SPI バス**（#157）なので転送時間は足し算。
全画面 320×240×16bit = 153,600B の転送だけで 40MHz なら約 31ms＝30fps 予算の 92%。もしそうなら
**全画面のままの 30fps は原理的に不可能**で解像度を落とすしかない（`bench` の push と clk で判明する）。

なお **30fps が無理でも実害は無い**（現状 10fps で体感上の問題なし）。その場合は
「全画面では 12〜21fps が上限」という事実を記録して閉じてよい。測り終えたら計装を残すか
捨てるかも判断する（#169 の計装は間引きで再生を乱さない形で残してある。同じ流儀）。
