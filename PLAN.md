...
⚠ Issue の状態が実態とずれているものがある: **#169**（原因特定は完了・修正 #170 も実機確認済みなのに
OPEN のまま）、**#184**（この PLAN と同じ PR で対応）。squash マージでは `Closes #N` が効かず
Issue が OPEN に残るのが repo 横断で再現している（手動 close が要る）。
