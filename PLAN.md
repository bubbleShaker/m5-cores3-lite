...
### 🔴 未確定: 全長素材が PSRAM に載るか（2c-2 の要否）
今回の実測は**冒頭30秒クリップ**なので、音声は 960KB で PSRAM に余裕で載った。
全長 236 秒だと **16kHz mono 16bit で約 7.5MB** となり、8MB の PSRAM にはほぼ載らない見込み。
2c-1 は WAV 全体を `ps_malloc` する実装なので、ここで確保に失敗すると無音になる。
**全長版を転送して試すのが 2c-2（チャンクストリーミング）の要否を決める次の分岐点。**
載らなければ `--sample-rate` を下げて延命するか、2c-2 に進むかを判断する。
