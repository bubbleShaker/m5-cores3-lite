...
### 🔴 次にやること（どちらから始めてもよい・Issue は起票済み）

**#175 動画を選べるようにする**
今は `kVideoDir` が `/video/sample` 固定。SD の `/video/` 直下を列挙して選択画面を出す。
既存の「声の選択」（src/voice_select.cpp）が同じ「リストから選ぶ」パターンを持っているので倣う。
カーソル移動の純粋ロジック `menu_move` も既にある。列挙自体は数個のエントリなら FAT32 でも一瞬
（遅かったのは 2,355 個の走査であって、ディレクトリ列挙そのものではない）。
