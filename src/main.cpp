...
constexpr uint16_t kColPacGhost  = TFT_RED;        // ゴースト（Step4）
...
// ゴーストを (tx,ty) の升目中心に赤い丸で描く（Step4）。
static void pacDrawGhost(int tx, int ty) {
    const int cx = kPacOx + tx * kPacTile + kPacTile / 2;
    const int cy = kPacOy + ty * kPacTile + kPacTile / 2;
    M5.Display.fillCircle(cx, cy, kPacTile / 2 - 2, kColPacGhost);
}
...
constexpr int kMenuRowH     = 24;  // 行間（シーン増加に合わせ 26→24 に詰めた・#134）
