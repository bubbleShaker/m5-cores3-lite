...
// という状態遷移と輝度の算出、および「その輝度を1画素へ適用する計算」までを扱う。
// 描画・SD・時計には依存しない（millis 由来の now を引数で受け取るだけ）ので native
// テストできる（scene/menu/video_frame_at と同じ作法）。画素の減光をここに同居させるのは、
// 輝度の定義（下記 0..256）と適用が離れていたせいで #213 の色化けが実機まで残ったため。
//
// 輝度は 0..256 の 257 段（256 = 等倍）。8bit の 255 ではなく 256 を等倍にするのは、
// video_fade_scale565 のスケール計算 (c * level) >> 8 が level=256 で厳密に恒等になるため。
...
// RGB565 の1画素を輝度 level/256 で暗くする（level は video_fade_level の 0..256）。
// 入力・出力ともネイティブ順の RGB565（swap565 の複写側で bswap してから渡す）。
//
// R/G/B を「取り出してから」掛けるのが要点（Issue #213）。R と B を 0xF81F でまとめて
// 掛ける定石は 5bit アルファ（>>5）用で、0..256 の level（>>8）では成立しない:
//   (R*2048 + B) * level >> 8  =  R*8*level + B*level/256
// R 側の項は 8 の倍数どまり＝ B フィールドの bit3/bit4 が立ち、それを & 0xF81F が青として
// 拾う（例: R=15,B=15,level=130 で B が 7 のはずが 23）。暗転の途中だけ画面が青に寄る。
//
// ⚠ 切り捨て（丸めを足さない）ことに依存した性質がある: level<256 では各チャンネルが
// 最大値に届かないので、結果が透明キー 0xF81F と一致しない。呼び出し側（videoDiscFadeApply）は
// これに寄りかかって「スケール後の画素がキーと衝突して穴が空く」ことを防いでいる。
// 丸め（+128 など）を入れると level=255 で R=31 に到達して壊れる。テストで固定してある。
