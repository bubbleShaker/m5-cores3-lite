...
// 面の外向き法線（正規化）。法線 = (b-a)×(c-a) を単位長に正規化して返す。
//   頂点を表向き（外から見て反時計回り）で定義してあれば、この法線は立体の外を向く。
//   面が退化（3点が一直線で面積0）して法線長が0なら {0,0,0} を返す（安全側・明るさ0扱い）。
//   フラットシェーディングで「面がどちらを向くか」を1本のベクトルで代表させるために使う。
Vec3 gem3d_face_normal(const Vec3& a, const Vec3& b, const Vec3& c);

// 面の明るさ係数 [0,1]。面法線 normal と「光源へ向かう方向」light の cos（正規化内積）を返す。
//   光が面の正面から当たるほど1に近づき、真横で0、裏から当たる（内積が負）と0にクランプする。
//   normal/light はここで内部正規化するので呼び出し側で正規化不要。どちらかが零ベクトルなら0。
//   描画側はこの係数に環境光を足して宝石色のシェード率に変換する（環境光・配色は実機描画層の責務）。
