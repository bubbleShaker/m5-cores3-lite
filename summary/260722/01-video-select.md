...
## レビュー（reviewer サブエージェント・2 回目 = 実機確認後）

🔴 must 1 件を修正した。

**決定タップ直後に偽の「一周」が発火する回帰**。`loop` は先頭で `now` を取り、タップ委譲の
**後に同じ `now` で** `update` を呼ぶ。決定タップで `videoStartPlayback` が走ると、その中の
`g_videoEnterMs = millis()` が `now` より後の時刻になる（索引読み＋音声 3〜7MB のロードで
数百ms〜数秒かかるので事実上必ず）。すると `elapsed = now - g_videoEnterMs` が uint32 で
アンダーフローして約 42 億 ms になり、`videoRestartCycle` が誤発火して音の頭が切れ、
`[video] wrap` にデタラメな計測が 1 件混ざっていた。旧実装ではシーン入場が `g_inMenu` 分岐内で
`return` していたため存在しなかった経路で、#175 で入り込んだ回帰。

修正は **時間軸を解釈する唯一の場所**（`videoUpdate` 冒頭）に閉じ、符号付きで比較する:

```cpp
if (static_cast<int32_t>(now - g_videoEnterMs) < 0) return;
```

呼び出し側に return やフラグを散らす案より、経路が増えても壊れない。millis の 49.7 日ラップにも
同じ形で効く。実機ログで `[video] wrap` 8 件すべてが正常値（101,610〜101,629ms / 公称 101,691ms）に
なったことを確認済み。

あわせて 🟡 を 2 件:
- SD マウント失敗時に `video_list_clear` を呼ぶ（前回入場の候補が残り、誤った理由が出るのを防ぐ）
- `videoFailToSelect` で解放前に `M5.Speaker.stop()`（`videoReleaseAudio` の前提をコードで閉じる）

残った 🟡/🟢（basename 正規化の純粋関数化とテスト、`dropped` の画面表示、重複ロジックの共通化）は
別 Issue に分離した。

## レビュー（reviewer サブエージェント・1 回目 = 実装直後）
...
- [x] SD に 2 本以上置いて実機で選んで再生できる（2026-07-22 実施。`bluelight`/`lain` の 2 件が
      列挙され、切り替えて再生できることを実機で確認）
- [x] 動画を切り替えても PSRAM が確実に返る（2026-07-22 シリアルログで数値確認・下表）

### PSRAM 返却の実測（`audio-alloc-before`）

| 入場 | psram_free |
|---|---|
| 1 回目 `lain` | 8,320,431 |
| 2 回目 `lain`（別シーン経由で再入場） | **8,320,431**（1 バイトも違わない） |
| 3 回目 `bluelight` | 8,308,231（−12,200） |

`bluelight` の 12,200 バイト差はリークではなく、**素材差による既確保分**で説明がつく:
pack 索引 18,840B−8,128B=10,712 ＋ 最大 JPEG バッファ 16,638B−15,151B=1,487 = 12,199
（差 1 バイトはアロケータのアラインメント）。同一素材の再入場が完全同値であることが決定的。

## 実機転送で実施したこと（2026-07-22）

1. MSC ファームを COM3 経由で書き込み（通常ファームからなら自動リセットが効く）→ D: にマウント。
2. `video/lain/` を `D:\video\lain` へコピー。3 ファイルとも `Get-FileHash` 一致を確認。
3. 既存の `D:\video\sample` を **`bluelight` にリネーム**（中身は無変更・33,626,111 bytes のまま）。
   `sample` という名前は `kVideoDir` が `/video/sample` 固定だった時代の名残で、中身とは無関係だった。
   本 Issue で名前の自由が生まれたので、素材の実体に合わせて改名した。
4. 通常ファーム（`pio run -e m5stack-cores3 -t upload`）へ焼き戻し（RST 3 秒長押しが必要）。

⚠ ローカルの `video/` 配下には過去の変換物（`bluelight`=30秒の旧 JPEG 連番形式、`packed`、`full`）が
残っているが、**SD 上の `bluelight` はこれらとは別**（SD 側は全長 2355 フレームの pack 素材）。
ローカルからの再コピーで SD を上書きしないこと。
