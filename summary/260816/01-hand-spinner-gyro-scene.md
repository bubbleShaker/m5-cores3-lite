...
文字は 4 隅に振り分けている。右下に `現在rpm  累計回転数`（回転中のみ）、右上に `最高 N rpm`。
最高 rpm は**止まった後も残り、次に回し始めた瞬間**に取り直す。止まった直後こそベストを
読みたいタイミングなので、停止と同時に消してはいけない。
...
### 5. 右下の文字が左下のヒントを黙って消す（レビューで発見）

「最高 N rpm」を右下の同じ行に足したところ、幅が 88px 増えて左のヒントに 72px 食い込んだ。
`setTextColor(前景, 背景)` は**背景ごと塗る**ので、後から描いた側が先に描いた文字を消す。
`lgfxJapanGothic_16` は全角 16px・半角 8px なので、左右で同じ行を使うなら
「左の文字数 × 16 + 右の文字数 × 16 < 320」を守る必要がある。今は 4 隅に振り分けた。
...
- `kSpMaxOmega`(2880 deg/s) が BMI270 のフルスケール設定を超えている可能性がある。
  超えていれば上限に到達しないだけで害は無いが、高速回転でコマ送りが破綻しないか見ておく
