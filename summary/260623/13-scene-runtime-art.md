...
## 付随修正：羊のちらつき（Issue #41）

実機目視で羊シーンが**ちらつく**事象を確認。原因は `drawSheep()` が毎フレーム
**画面に直接**「クリップ枠を消去 → 描き直す」を行い、消去直後の背景だけの瞬間が見えていたこと。

**オフスクリーン・キャンバス（`M5Canvas` によるダブルバッファ）** で解消：
メモリ上のキャンバスに全部描いてから `pushSprite` で**1回の転送で画面へ反映**する
（途中の消去状態を画面に出さない）。

- キャンバスは初回 `sheepEnter()` で `createSprite(208x170)` ＝約70KB（内部RAM に収まる）
- `drawSheep` / `drawSheepEye` の描画先を `M5.Display` → `M5Canvas` に変更し、座標を枠原点基準のローカル座標へ
- 純粋ロジックは不変。実機で滑らかな揺れを確認済み

## 関連 Issue / PR

- Issue: #33（シーン状態機械）, #34（アート M2 実機描画）, #37（実機目視チェック）, #41（羊ちらつき修正）
