...
| `src/config.ts` | 純粋。環境変数の解釈（dry-run の既定・タイムアウトの範囲検証） |
...
| `RELAY_EXEC_TIMEOUT_MS` | `10000` | PowerShell が無応答の時に諦めるまで（1〜2147483647 の範囲外は既定値へ） |
...
| コード | 意味 | 返す outcome |
|---|---|---|
| 0 | 成功（実行パスが一致した窓を操作） | `ok` |
| 3 | 対象の窓が無い | `not_running` |
| 5 | 成功（実行パスは不一致・名前一致の 1 件を操作） | `ok` + `detail: 名前一致で実行` |
| その他 | 失敗（引数の束縛失敗を含む） | `failed` |

既知の制約:

- **ウィンドウの特定はプロセス名 + 実行パス。** `Get-Process -Name` はベース名一致なので、
  実行パスが一致するものがあればそこまで絞る。一致が 0 件で、**名前一致の候補がちょうど
  1 つのときだけ**それを操作する（`exit 5`）── Windows 11 の `notepad.exe` のように
  **起動したパスと実プロセスのパスが違う**アプリがあるため（Store 版へリダイレクトされ
  実体は `WindowsApps\...\Notepad.exe`）。候補が複数ある時はフォールバックしない
  （同一性が最も怪しい状況で最も広く手を出すことになるため）。
  - 同じ理由で 8.3 短縮名（`C:\PROGRA~1\...`）やジャンクション経由のパスも一致せず、
    静かにフォールバック側へ落ちる。厳密一致させたい場合は実プロセスのパス
    （`Get-Process -Name x | Select-Object Path`）を `apps.json` に書く。
    ただし Store アプリはバージョン更新でパスが変わる。
- **`launch_app` の `ok` は「起動できた」まで。** 直後にアプリが自分で落ちても成功と答える。
  起動がタイムアウトした場合も**そのアプリは kill しない**（ユーザーが開こうとした窓を
  relay が勝手に閉じる方が驚きが大きい）。PowerShell 側は kill する。
- **同時に走る操作は 1 つ。** 実行中の操作要求は `busy`（プロセスの無制限増殖を防ぐため）。
  `denied`（許可リスト違反）とは別の記号にしてあるので、監査ログで切り分けられる。
...
- `src/winexec.ts` / `src/audit.ts` / `src/config.ts` — 純粋ロジック（実行パス検証・PowerShell 引数組み立て・監査ログ整形・環境変数の解釈）。
